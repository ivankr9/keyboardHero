...
bg = pg.image.load(dir_and_name[0] + "/data/background3.png")
...
screen = pg.display.set_mode((700, 330))
...
def play_bass_note(picth, bass_sounds, bass_to_activate_list):
    ...
    rnd_koef = random.uniform(0.1, 0.24)
...
def play_key_note(pitch_note, keys_to_activate_list, keys_type_sustain_list, key_sounds ):
    ...
    rnd_koef = random.uniform(0.2, 0.5)
...
while run:
    for event in pg.event.get():
        ...
        if event.type == pg.KEYDOWN and (event.key == pg.K_RCTRL):
            bass_toggle = True
            to_print = 'BASS MODE              ' + to_print
        if event.type == pg.KEYUP and (event.key == pg.K_RCTRL):
            bass_toggle = False
            to_print = 'BASS MODE OFF              ' + to_print
            
        
        if event.type == pg.KEYDOWN and event.key == pg.K_BACKQUOTE:
            if rand_toggle:
                rand_toggle = False
                to_print = 'OCTAVE LOW MODE OFF              ' + to_print
                hi_octave_mode = False
                octave_shift = -1
                #to_print = '              ' + to_print
            else:
                rand_toggle = True
                to_print = 'octave HI MODE ON              ' + to_print
                hi_octave_mode = True
                octave_shift = 0
                #to_print = '              ' + to_print
        ...
        if event.type == pg.KEYDOWN and event.key == pg.K_BACKSPACE: # MUTE BACKSPAce
        ...
        if event.type == pg.KEYUP and event.key == pg.K_BACKSPACE:   # UN MUTE
            ecs_pressed_mute = False
        ...
        if event.type == pg.KEYDOWN and event.key == pg.K_ESCAPE: # START STOP BASS
            if metronome_is_play:
                metronome_is_play = False
        ...
        if  event.type == pg.KEYDOWN and (event.key  == pg.K_LEFT or event.key == pg.K_END or event.key == pg.K_SPACE): # BASS DRUM
            rnd_koef = random.uniform(0.26, 0.47)
            drum_sounds[0].set_volume(rnd_koef*mult_vol)
            drum_sounds[0].play()

        if event.type == pg.KEYDOWN and (event.key == pg.K_RIGHT or event.key == pg.K_UP or event.key == pg.K_LCTRL): # SNARE
            rnd_koef = random.uniform(0.14, 0.24)
            drum_sounds[2].set_volume(rnd_koef*mult_vol)
            drum_sounds[2].play()
            
        if event.type == pg.KEYDOWN and (event.key == pg.K_DOWN or event.key == pg.K_LALT): # HIHAT OPEN
            #rnd_koef = random.uniform(0.78, 0.86)
            drum_sounds[7].set_volume(0.19*mult_vol)
            drum_sounds[7].play()

        if event.type == pg.KEYDOWN and (event.key == pg.K_RALT or event.key == pg.K_TAB): #: # HIHAT CLOSED
            rnd_koef = random.uniform(0.21, 0.45)
            drum_sounds[1].set_volume(rnd_koef*mult_vol)
            drum_sounds[7].stop()
            drum_sounds[1].play()

        if event.type == pg.KEYDOWN and event.key == pg.K_LSHIFT: # RING to Alt from space
            rnd_koef = random.uniform(0.2, 0.35)
            drum_sounds[3].set_volume(rnd_koef*mult_vol)
            drum_sounds[3].play()
        ...
        # if event.type == pg.KEYDOWN and event.key == pg.K_PERIOD:### need
        #     play_bass_note(24, bass_sounds, bass_to_activate_list) # hi D
        # if event.type == pg.KEYDOWN and event.key == pg.K_SLASH:      # pg.K_SEMICOLON
        ...
        # #semitones
        # if event.type == pg.KEYDOWN and event.key == pg.K_MINUS:    #K_PERIOD
        ...
        if event.type == pg.KEYDOWN and event.key == pg.K_t:
            ...
            if bass_toggle:
                play_bass_note(25, bass_sounds, bass_to_activate_list) # D#m
        ...
        if event.type == pg.KEYDOWN and event.key == pg.K_u:
            ...
            if bass_toggle:
                play_bass_note(18, bass_sounds, bass_to_activate_list) # G#m
        ...
        if event.type == pg.KEYDOWN and event.key == pg.K_d:
            play_key_note(31, keys_to_activate_list, keys_type_sustain_list, key_sounds )
            if bass_toggle:
                play_bass_note(21, bass_sounds, bass_to_activate_list) # bm    
        ...
        if event.type == pg.KEYDOWN and event.key == pg.K_g:
            play_key_note(24, keys_to_activate_list, keys_type_sustain_list, key_sounds )
            if bass_toggle:
                play_bass_note(14, bass_sounds, bass_to_activate_list) # em
        ...
        if event.type == pg.KEYDOWN and event.key == pg.K_c:
            play_key_note(32, keys_to_activate_list, keys_type_sustain_list, key_sounds )
            if bass_toggle:
                play_bass_note(22, bass_sounds, bass_to_activate_list) # cm
        ...
        if event.type == pg.KEYDOWN and event.key == pg.K_v:
            ...
            if bass_toggle:
                play_bass_note(18, bass_sounds, bass_to_activate_list) # g#
        if event.type == pg.KEYUP and event.key == pg.K_v:
            realize_key_note(28, keys_to_activate_list)

        if event.type == pg.KEYDOWN and event.key == pg.K_b:
            play_key_note(25, keys_to_activate_list, keys_type_sustain_list, key_sounds )
            if bass_toggle:
                play_bass_note(15, bass_sounds, bass_to_activate_list) # fm
        ...
        if event.type == pg.KEYDOWN and event.key == pg.K_n:
            ...
            if bass_toggle:
                play_bass_note(23, bass_sounds, bass_to_activate_list) # c#
        if event.type == pg.KEYUP and event.key == pg.K_n:
            realize_key_note(33, keys_to_activate_list)

        if event.type == pg.KEYDOWN and event.key == pg.K_m:
            play_key_note(30, keys_to_activate_list, keys_type_sustain_list, key_sounds )
            if bass_toggle:
                play_bass_note(20, bass_sounds, bass_to_activate_list) # a#m
        ...
        if event.type == pg.KEYDOWN and event.key == pg.K_SLASH:
            ...
            if bass_toggle:
                play_bass_note(21, bass_sounds, bass_to_activate_list) # b
        if event.type == pg.KEYUP and event.key == pg.K_SLASH:
            realize_key_note(31, keys_to_activate_list)
